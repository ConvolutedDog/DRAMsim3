...
SimpleStats::SimpleStats(const Config &config, int channel_id)
  ...
  InitHistoStat("interarrival_latency", "Request interarrival latency (cycles)",
                0, 100, 10);
...
void SimpleStats::PrintFinalStats() {
  ...
  if (config_.output_level >= 1) {
    ...
    for (const auto &it : print_pairs_) {
      PrintStatText(txt_out, it.first, it.second, header_descs_[it.first]);
    }
  }
...
void SimpleStats::UpdateFinalStats() {
  ...
  for (int i = 0; i < config_.ranks; i++) {
    ...
    double pre_stb =
        vec_counters_["all_bank_idle_cycles"][i] * config_.pre_stb_energy_inc;
    double sref_energy =
